...
%             'string',strings2{1},'callback',@manavv,'backgroundcolor','red','tag','bu1');
        ...
        uicontrol('Units','pixels','Position',[100 400 20 20],...
         'String','E','style','text','foregroundcolor','w','fontsize',9,'backgroundcolor','r','tag','V');
...
%           ince=solve(oo,xx)
         ...
         R3_W = [ cos(RA*pi/180) sin(RA*pi/180) 0
            ...
            R0=r';
%             X0=[R00;V0];
            E = norm(V0)^2/2 - mu/norm(R00)==x;
             
            E= double(vpasolve(E,dV));
         manev(E,0,0,S);
 solver=sprintf(' delta V in velDirection = %d',E);
(msgbox(solver));
